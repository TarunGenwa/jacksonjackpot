...
export class DrawService {
  ...
  async commitSeed(dto: CommitSeedDto) {
    ...
    if (competition.status !== CompetitionStatus.ACTIVE && competition.status !== CompetitionStatus.SOLD_OUT) {
      throw new BadRequestException(`Cannot commit seed for competition in ${competition.status} status`);
    }

    try {
      const result = await this.prizeAllocationService.commitDrawSeed(dto.competitionId, dto.seed);
      return {
        success: true,
  ...
  async revealSeed(dto: RevealSeedDto) {
    ...
    try {
      const result = await this.prizeAllocationService.revealDrawSeed(dto.competitionId, dto.seed);
      return {
        success: true,
        drawSeedId: result.drawSeed.id,
      ...
      if (error.message.includes('does not match') || error.message.includes('already revealed')) {
        throw new BadRequestException(error.message);
      }
      throw error;
    }
  ...
  async executeDraw(dto: ExecuteDrawDto) {
    ...
    if (!competition.drawSeed || !competition.drawSeed.seedReveal) {
      throw new BadRequestException('Seed must be revealed before executing draw');
    }

    if (competition.prizes.length === 0) {
      throw new BadRequestException('No prizes configured for this competition');
    }
    ...
    try {
      const result = await this.prizeAllocationService.executeCompetitionDraw(dto.competitionId);
  ...
  async getDrawStatus(competitionId: string) {
    ...
    return {
      ...
      seedStatus: competition.drawSeed ? competition.drawSeed.status : 'NOT_COMMITTED',
  ...
  async validateDrawIntegrity(competitionId: string) {
    ...
    const chainEntries = await this.hashChainService.getEntriesByType('DRAW_RESULT');
    const drawEntry = chainEntries.find(e =>
      (e.data as any).competitionId === competitionId
    );
